...
const VirtualizedMUIGridStyles = ({ palette }: Theme) => {
  return createStyles<VirtualizedMUIGridClassKey, VirtualizedMUIGridProps>({
    root: {
...
export const VirtualizedMUIGrid = forwardRef<any, VirtualizedMUIGridProps & { component?: React.ElementType }>(
  (
    {
      classes = {},
      columns = 1,
    ...
    useEffect(() => {
      const rowArray = [];
      const totalRows = Math.ceil(data.length / (variant === 'List' ? 1 : columns));
    ...
    const renderRow = (row: number) => {
      const filterRowData = data.slice(currentRenderCount, currentRenderCount + columns);
      ...
      return (
        <VirtualItem
          ...
          offset={(containerRef?.current?.clientHeight || 0) + rowHeight * preRenderRowCount}
          spacing={spacing}
